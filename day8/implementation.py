...
def solver(filename: str):
    ...
    for i in range(1, n_rows - 1):
        for j in range(1, n_columns - 1):
            ...
            scenic_scores_coordinates.append((i, j))
            if i == 49 and j == 85:
                print(scenic_score)
    n_visible_trees += 2 * n_rows + 2 * n_columns - 4
    scenic_score = max(scenic_scores)
    print(scenic_score)
    scenic_score_index = np.where(np.array(scenic_scores) == scenic_score)
    print(scenic_score_index[0])
    best_tree = scenic_scores_coordinates[scenic_score_index[0][0]]
    print(best_tree)
    print(data_matrix[49, 85])
